...
using
    Newtonsoft.Json; //Used to deserialize complex JSON --> https://code-maze.com/csharp-deserialize-complex-json-object/
...
public class Inventory : MonoBehaviour
{
        ...
        try
        {
            ...
            foreach (var nftOwner in nftOwners)
            {
                if (nftOwner.Metadata == null)
                {
                    ...
                    await Moralis.GetClient().Web3Api.Token
                        .ReSyncMetadata(nftOwner.TokenAddress, nftOwner.TokenId, contractChain);
    ...
    public void DeleteItem(string id)
    {
        ...
        {
            InventoryItem
                itemClass = item.GetComponent<InventoryItem>(); // Assuming every item has InventoryItem script
